...
def main() -> None:
    ...
    if settings["solution open"] == settings["solution close"]:
        ...
        print(
            f"  current setting: {settings['solution open']} / {settings['solution close']}"
        )
        quit()
...
def create_zip(path: str, settings: configparser.SectionProxy) -> None:
    ...
    if settings.getboolean("create zip") or settings.getboolean("create zip only"):
        ...
        if os.path.isfile(outfile) and not DRY_RUN:
            ...
            logger.warning(
                f"  directory found at {outfile}! unable to create zip file."
            )
            return
        ...
        if not DRY_RUN:
            try:
                ...
                logger.warning(
                    f"  could not create zip at {outfile} ({oserr.strerror})"
                )
                return
